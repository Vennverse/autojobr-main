...
export default function PayPalMockInterviewPayment({ 
  ...
  const handleAmazonPayPayment = async () => {
    ...
    try {
      // Check if Amazon Pay credentials are available
      const response = await fetch('/api/payment/amazon-pay/check-credentials');
      const data = await response.json();
      
      if (!data.available) {
        toast({
          title: "Amazon Pay Not Available",
          description: "Amazon Pay integration is not configured yet. Please use PayPal or contact support.",
          variant: "destructive",
        });
        return;
      }
