...
import PayPalSubscriptionCard from "./PayPalSubscriptionCard";
...
export default function UsageMonitoringWidget() {
  ...
  const getFreeRankingTestData = () => {
    if (usageReport && typeof usageReport === 'object' && 'usage' in usageReport && 'limits' in usageReport) {
      const freeUsed = (usageReport as any).usage?.freeRankingTests || 0;
      const freeLimit = (usageReport as any).limits?.freeRankingTests || (isPremium ? 1 : 0);
  ...
  const report: UsageReport = usageReport || { subscription: null, usage: {}, limits: {}, percentages: {}, upgradeRecommended: false };
