...
// Get user's virtual interview history
router.get('/history', isAuthenticated, async (req: any, res) => {
  try {
    ...
    const formattedInterviews = interviews.map(interview => ({
      ...interview,
      strengths: interview.strengths ? JSON.parse(interview.strengths as string) : [],
      weaknesses: interview.weaknesses ? JSON.parse(interview.weaknesses as string) : [],
      recommendations: interview.recommendations ? JSON.parse(interview.recommendations as string) : []
    }));

    res.json(formattedInterviews);
  } catch (error) {
    console.error('Error fetching virtual interview history:', error);
    res.status(500).json({ message: 'Failed to fetch interview history' });
  }
});

// Get chat interview feedback
