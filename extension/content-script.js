...
class AutoJobrContentScript {
  ...
  init() {
    ...
    try {
      // Load settings first (removed insecure userApiKey storage - now handled server-side)
      chrome.storage.sync.get(['premiumFeaturesEnabled'], (result) => {
        // API keys are now managed securely on the server
        this.groqApiKey = null; // Deprecated - server handles API keys
  ...
  isJobPage() {
    ...
    // Google Forms and other form platforms (application pages)
    const formPlatforms = {
      'docs.google.com/forms': true,
      'forms.gle': true,
      'typeform.com': pathname.includes('/to/'),
      'surveymonkey.com': pathname.includes('/r/'),
      'jotform.com': true,
      'formstack.com': true,
      'wufoo.com': true,
      'cognito forms.com': true,
      'formsite.com': true,
      'forms.office.com': true, // Microsoft Forms
      'airtable.com': pathname.includes('/form'),
    };

    for (const [domain, condition] of Object.entries(formPlatforms)) {
      if (hostname.includes(domain)) {
        if (condition === true || condition) {
          console.log(`📋 Application form detected on ${domain}`);
          return true;
        }
      }
    }
    ...
    // Enhanced fallback: check for generic job and form indicators in URL and DOM
    const genericJobIndicators = ['/job/', '/jobs/', '/career/', '/careers/', '/position/', '/apply/', '/posting/', '/job_', '/job-', '/application/', '/applications/', '/form/'];
